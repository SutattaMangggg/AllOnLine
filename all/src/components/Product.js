...
const Product = ({ product }) => {
  ...
  const { id, image, category, product_name, sell_price } = product;
  return (
    <div>
      ...
      {/* category & productname & sellprice */}
      <div>
        <div className='text-sm capitalize text-gray-500 mb-1'>{category}</div>
        <Link to={`/product/${id}`}>
        <h1 className='font-semibold mb-1'>{product_name}</h1>
        </Link>
        <div className='font-semibold'>฿ {sell_price}</div>
      </div>
    </div>
  );
};
